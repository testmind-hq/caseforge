...
- **测试用例评分** — 多维度质量评分，含命名覆盖场景追踪（广度覆盖、边界、安全、执行、状态码覆盖）
...
--include-path string   正则表达式，按路径筛选操作（例如 '^/users'）
--exclude-path string   正则表达式，按路径排除操作（例如 '^/admin'）
--include-tag string    包含的 OpenAPI 标签，逗号分隔（例如 'users,auth'）
--exclude-tag string    排除的 OpenAPI 标签，逗号分隔（例如 'deprecated'）
```
...
--format string       hurl | markdown | csv | postman | k6（默认：hurl）
--data-pool string    由 explore --export-pool 写出的 JSON 数据池文件
--seed-postman string 用于初始化数据池的 Postman Collection v2.1 JSON 文件
```

链式用例会读取 OpenAPI Links，将生产者 `$response.body` 字段自动注入消费者的路径/查询参数，
并为深度为 2 且消费者非 DELETE 操作的链自动追加 DELETE 清理步骤。
...
--dry-run                  只生成假设，不执行探测
--export-pool string       将 2xx 响应中观测到的字段值写入 JSON 数据池文件
--prioritize-uncovered     两轮调度：第一轮对所有操作各执行一次探测（广度扫描），
                           第二轮将剩余预算集中在未返回 2xx 的操作上
--max-failures int         发现 N 条规则后停止探测（0 = 不限）
--include-path string      正则表达式，按路径筛选操作（例如 '^/users'）
--exclude-path string      正则表达式，按路径排除操作（例如 '^/admin'）
--include-tag string       包含的 OpenAPI 标签，逗号分隔（例如 'users,auth'）
--exclude-tag string       排除的 OpenAPI 标签，逗号分隔（例如 'deprecated'）
...
`--export-pool` 写出的数据池文件可通过 `caseforge chain --data-pool` 加载，
为链式探测注入真实的字段值。
